...
const RechargePage = (props) => {
  ...
  const [email,setEmail]=useState(JSON.parse(localStorage.getItem('auth')).user.email);
  const [budget,setBudget]=useState(JSON.parse(localStorage.getItem('auth')).user.budget);
  const [money,setMoney]=useState('');
  const [firstname,setFirstname]=useState(JSON.parse(localStorage.getItem('auth')).user.firstname);
  ...
  const apply=async ()=>{
      ...
      if(response.status==200){
        ...
        var ttt=JSON.parse(localStorage.getItem('auth'));
