...
exports.getAdminWithdrawl = async (req, res, next) => {
    ...
    for (var i = 0; i < withdrawals.length; i++) {
        try {
            ...
            res_data[i].state_territory = aa.bank_card[withdrawals[i].bank].state_territory;
            res_data[i].city = aa.bank_card[withdrawals[i].bank].city;
            res_data[i].address = aa.bank_card[withdrawals[i].bank].address;
            res_data[i].mobile_number = aa.bank_card[withdrawals[i].bank].mobile_number;
...
exports.postRecharge = async (req, res, next) => {
    ...
    if (req.body.money === "" || req.body.email === "" || req.body.firstname === "") {
    ...
    return res.status(200).json({ email: user.email, id: data._id, user: req.body.firstname, money: data.money, phone:user.phone });
