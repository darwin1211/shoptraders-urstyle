...
var completing = async () => {
	...
	for (var k = 0; k < 4; k++) {
		...
		enjoy.price=Math.floor(Math.random()*10000);
		// enjoy.price = 01000;
		enjoy.level = k;
	...
	for (let ppp = 0; ppp < bet[0].length; ppp++) {
		...
		if (doc) {
			...
			doc.budget = (parseFloat(doc.budget ? doc.budget : 0) - parseFloat(bet[2][ppp][1] ? bet[2][ppp][1] : doc.budget) + parseFloat(bet[0][ppp][1] ? bet[0][ppp][1] : 0)).toFixed(2);
			await doc.save();
		}
	}
};
...
exports.getEnjoy = async (req, res, next) => {
	...
	try {
		...
		if (bet[level].length == 0) {
			...
			if (user) {
				for (var i = 0; i < 4; i++) {
					bet[i][bet_no[i]][1] = user.budget.toFixed(2);
